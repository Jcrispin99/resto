...
const props = defineProps<{
    ...
    units: Unit[];
    selectedProductTemplateId?: number;
}>();

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Dashboard', href: dashboard().url },
    { title: 'Recipes', href: '/recipes' },
    { title: 'Create', href: '/recipes/create' },
];

const form = useForm({
    product_template_id: props.selectedProductTemplateId?.toString() || '',
    ingredient_id: '',
    quantity: 0,
    unit_id: '',
    waste_percentage: 0,
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Define the ingredients needed for this dish</CardDescription>
                        </CardHeader>
                        <CardContent class="space-y-4">
                            ...
                            <!-- Quantity & Unit -->
                            <div class="grid gap-2">
                                <Label>Quantity * <span class="text-sm font-normal text-gray-500">(per serving/portion)</span></Label>
                            ...
                            <!-- Waste Percentage -->
                            <div class="grid gap-2">
                                <Label for="waste">Waste / Shrinkage (%) <span class="text-sm font-normal text-gray-500">(optional)</span></Label>
                                <Input 
                                    id="waste"
                                ...
                                <p class="text-xs text-gray-500">
                                    Expected loss during preparation (cleaning, trimming, cooking)
                                </p>
                                ...
                                <div v-if="form.waste_percentage > 0" class="mt-1 p-3 bg-orange-50 border border-orange-200 rounded">
                                    <div class="text-sm space-y-1">
                                        <div class="flex justify-between">
                                            <span class="text-gray-700">Base quantity:</span>
                                            <span class="font-medium">{{ Number(form.quantity || 0).toFixed(3) }} {{ selectedUnit?.abbreviation }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-gray-700">Waste ({{ form.waste_percentage }}%):</span>
                                        ...
                                        <div class="flex justify-between pt-1 border-t border-orange-300">
                                            <span class="font-medium text-gray-900">Total needed:</span>
                                            <span class="font-bold text-orange-700">{{ Number(totalWithWaste || 0).toFixed(3) }} {{ selectedUnit?.abbreviation }}</span>
                                        </div>
                                    </div>
                                </div>
                                
                                <span v-if="form.errors.waste_percentage" class="text-sm text-red-500">
                                    {{ form.errors.waste_percentage }}
                                </span>
                            </div>

                            <!-- Notes -->
                            <div class="grid gap-2">
                                <Label for="notes">Notes <span class="text-sm font-normal text-gray-500">(optional)</span></Label>
                                <Textarea 
                                    id="notes"
                                    v-model="form.notes" 
                                    placeholder="e.g., Fresh fish from market, must be boneless"
                                    rows="3"
                                />
                                <span v-if="form.errors.notes" class="text-sm text-red-500">
                                    {{ form.errors.notes }}
                                </span>
                            </div>
                        </CardContent>
                    ...
                    <div class="flex justify-end gap-4">
                        ...
                        <Button type="submit" :disabled="form.processing">
                            {{ form.processing ? 'Creating...' : 'Create Recipe' }}
                        </Button>
                    </div>
                </form>
            </div>
